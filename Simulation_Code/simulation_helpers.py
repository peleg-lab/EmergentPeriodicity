...
def get_initial_interburst_interval():
    # Code for generating the envelope
    from scipy.interpolate import make_interp_spline
    import numpy
    import csv

    with open('../Experimental_Results/Corrected/01ff/ibs01ff.csv', newline='') as f:
        reader = csv.reader(f)
        data = list(reader)
    ...
    # write
    with open('../Experimental_Results/Corrected/envelope_01ff_corrected.csv', 'w') as fwrite:
...
def draw_from_input_distribution(num_draws):
    # Code for using the envelope
    with open('../Experimental_Results/Corrected/envelope_01ff_corrected.csv', newline='') as f:
        reader = csv.reader(f)
        data = list(reader)

    X_NICE = [float(d[0]) for d in data]
    Y_NICE = [float(d[1]) for d in data]
    Y_NICE_N = [y / sum(Y_NICE) for y in Y_NICE]
    Tc_input = numpy.random.choice(X_NICE, num_draws, p=Y_NICE_N)
    return Tc_input
...
def spike_analysis(exp_results, argv):
    ...
    y = []
    y_maybes = []
    datafile = 'experimental_data/Corrected/20ff/ibs20ff.csv'
...
def spike_analysis_diff(ff_count):
    ...
    y = []
    y_maybes = []
    datafile = 'experimental_data/Corrected/{}ff/ibs{}ff.csv'.format(ff_count, ff_count)
    with open(datafile, 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for i, row in enumerate(plots):
            x.append(i)
